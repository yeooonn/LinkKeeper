...
const page = async ({ params }: { params: Promise<{ slug: string }> }) => {
  ...
  if (isFilter) {
    ...
    /** `filename` API는 링크 title 조건 — 폴더 슬러그와 무관. 전체 조회 후 folderId로 필터 */
    query = `?filter=${encodeURIComponent("전체")}&userId=${user?.id}`;
  }
  ...
  const folderIdFromPath =
    slugParts.length >= 2 ? slugParts[slugParts.length - 1] : null;

  const filterdData =
    folderIdFromPath && !isFilter && !isURL
      ? landingData.filter((link) => link.folder.id === folderIdFromPath)
      : landingData;
